...
export class ImageCanvas extends AnnotationCanvas {
  ...
  set mediaInfo(val) {
    ...
    new Promise(async (resolve) => {
      ...
      if (Number.isInteger(this._videoFrame)) {
        ...
        fetchCredentials(
          `/rest/GetFrame/${val.id}?frames=${this._videoFrame}&quality=${display_size}`,
          {
            mode: "cors",
            credentials: "include",
            headers: {
          ...
          .then((imageBlob) => {
            if (
              this_id != this._videoObject.id ||
              this_frame != this._videoFrame
            ) {
              console.warn(
                `Video ID or Frame Fetch has been received out of order.`
              );
            }
            ...
            reader.addEventListener("load", function () {
              ...
              that
                .loadFromURL(reader.result, that._dims)
                .then(() => {
                  that._good = true;
                  that.refresh();
                })
  ...
  _loadFromMediaFiles() {
    ...
    for (let idx = 0; idx < this._mediaFiles.image.length; idx++) {
      if (
        this._mediaFiles.image[idx].mime == "image/avif" &&
        this._supportsAvif == true
      ) {
        best_idx = idx;
        break;
  ...
  refresh() {
    ...
    const promise = new Promise((resolve) => {
      if (this._draw.canPlay()) {
        this._draw.updateImage(
          this._roi[0],
          this._roi[1], //No clipping
          this._roi[2],
          this._roi[3], //Image size
          leftSide,
          0, //Place 'full-screen'
          sWidth,
          sHeight // Use canvas size
        );
        this.moveOffscreenBuffer(this._roi);
      } else {
        this._draw.pushImage(
          frame,
          this._imageElement,
          this._roi[0],
          this._roi[1], //No clipping
  ...
  loadFromURL(URL, dims) {
    ...
    // viewport at that)
    this._imageScale = (window.screen.width * 0.7) / dims[0];
    let viewportDims = [
      Math.round(dims[0] * this._imageScale),
